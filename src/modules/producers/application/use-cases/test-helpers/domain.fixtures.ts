...
export function makeCrop(overrides?: Partial<{ id: string; name: string }>): Crop {
  return new Crop(overrides?.id ?? 'crop-1', CropName.create(overrides?.name ?? 'Soja'));
}

export function makeHarvest(overrides?: Partial<{ id: string; year: string }>): Harvest {
  return new Harvest(
...
export function makeProducer(overrides?: Partial<{ id: string; name: string }>): Producer {
  return new Producer({
